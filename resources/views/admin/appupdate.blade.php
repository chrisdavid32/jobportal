...
<body>
      ...
      <div class="main-panel">
        <div class="content-wrapper">
          <div class="card">
            <div class="card-body">
              <h4 class="card-title">Aptitude test Result list</h4>
              <div class="row">
                <div class="col-12">
                @include('applicant.alerts')
                  <div class="table-responsive">
                    <table id="order-listing" class="table">
                      ...
                      <tbody>
                      {{!$app=App\appointment::all()}}
                        <tr>
                        @foreach($app as $infor)
                        <?php $info2 = App\applicant::where('email',$infor->useremail)->first();
                           
                          ?>
                            <td>{{$infor->appointmentid}}</td>
                            <td>{{ucwords($info2->last_name)}}</td>
                            <td>{{ucwords($info2->first_name)}},&nbsp;{{ucwords($info2->other_name)}}</td>
                            <td>{{ucwords($info2->gender)}}</td>
                            {{!$dt = App\post::where('postid',$info2['post'])->first()}}
