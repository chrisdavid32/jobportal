                ...
                <div class="tab-content tab-transparent-content pb-0">
                  ...
                  @include('applicant.alerts')
                    ...
                    <div class="col-md-5 grid-margin stretch-card">
                      <div class="card">
                        <div class="card-body">
                          <h4 class="card-title">Add Job Position</h4>
                          {!! Form::open(['url'=>'jobpost']) !!}
                         <div class="form-group row">
                            <label for="exampleInputUsername2" class="col-sm-4 col-form-label">Category</label>
                            <div class="col-sm-8">
                            {{!$data = App\category::all()}}
                              <select class="form-control" name="jobCategory">
                              <option value="" selected>Select Category</option>
                              @foreach($data as $cat)
                              <option value="{{$cat->categoryid}}">{{ucwords($cat->category_name)}}</option>
                              @endforeach
                              </select>
                            </div>
                            ...
                            <div class="form-group row">
                            <label for="exampleInputUsername2" class="col-sm-4 col-form-label">Position</label>
                            <div class="col-sm-8">
                              <input type="text" class="form-control" name="positions"  placeholder="Position Name">
                            </div>
                          </div>
                          <div class="form-group row">
                    <div class="col-lg-4">
                    ...
                    <div class="col-lg-8">
                      <textarea id="maxlength-textarea" class="form-control" rows="7" name="positionDescription" placeholder="Enter Position Description"></textarea>
                    </div>
                  </div>
                              ...
                              <div class="table-responsive">
                                ...
                                {{!$dat = App\post::all()}}
                                  ...
                                  @foreach($dat as $job)
                                      <td>{{$sn++}}</td>
                                      <td>{{$job->postid}}</td>
                                      {{!$cat=App\category::where('categoryid',$job->cartid)->first()}}
                                      <td>{{ucwords($cat->category_name)}}</td>
                                     <td>{{ucwords($job->post)}}</td>
                                      <td>
                                      <a href="deleteposition/{{$job->postid}}" title="Delete {{ucwords($cat->post)}}">
                                          <h4>
